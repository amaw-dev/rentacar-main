 ...
 * injected into the browser via page.addScriptTag().
 *
 * The script exposes window.__devBrowser_getAISnapshot() and
  ...
  function getElementAccessibleName(element, includeHidden) {
    ...
    if (accessibleName === undefined) {
      ...
      if (!elementProhibitsNaming) {
        accessibleName = normalizeWhiteSpace(getTextAlternativeInternal(element, {
          includeHidden,
...
function getAriaSnapshotCode(): string {
  ...
  function renderAriaTree(ariaSnapshot) {
    ...
    const getSingleInlinedTextChild = (ariaNode) => {
      return ariaNode?.children.length === 1 && typeof ariaNode.children[0] === "string" && !Object.keys(ariaNode.props).length
        ? ariaNode.children[0] : undefined;
    };
